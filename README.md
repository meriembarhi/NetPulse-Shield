...
```
NetPulse-Shield/
├── detector.py             # Isolation Forest anomaly detector with joblib persistence
├── solver.py               # RAG pipeline for remediation advice
├── dashboard.py            # Interactive Streamlit interface
├── clean_data.py           # Filters the raw dataset into a usable size
├── models/                 # Saved model artifacts generated by detector.py
...
10% most anomalous-looking records), persists the trained model and scaler with
`joblib` in `models/`, and prints the top-5 most anomalous records.
...
- Traffic Data: Exploratory Data Analysis (EDA) tools to inspect the cleaned dataset and visualize feature distributions such as packet size and duration.
